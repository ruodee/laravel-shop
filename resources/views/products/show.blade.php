            ...
            <div class="card-body product-info">
                        ...
                        @if($product->type === \App\Models\Product::TYPE_CROWDFUNDING)
                            <div class="crowdfunding-info">
                                ...
                                <div>此项目必须在
                                    ...
                                    的支持才可成功，
                                    <!-- Carbon 对象的 diffForHumans()方法可以计算出当前时间的相对时间，更人性化 -->
                        ...
                        <div class="skus"><label>选择</label>
                            <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                @foreach($product->skus as $sku)
                                    ...
                                    <input type="radio" name="skus" autocomplete="off" value="{{ $sku->id }}">{{ $sku->title }}</label>
                                @endforeach
                            </div>
                        </div>
                ...
                <div class="product-detail">
                    <ul class="nav nav-tabs" role="tablist">
                        <li class="nav-items"><a href="#product-detail-tab" aria-controls="product-detail-tab" role="tab" data-toggle="tab" aria-selected="true" class="nav-link active">商品详情</a></li>
                        <li class="nav-items"><a href="#product-reviews-tab" class="nav-link" aria-controls="product-reviews-tab" role="tab" data-toggle="tab" aria-selected="false">用户评价</a></li>
                    </ul>
...
<script>
    $(document).ready(function () {
        ...
        $('.btn-favor').click(function () {
            // 发起一个post ajax 请求，请求 url 通过后端的 route() 函数生成。
            axios.post('{{ route('products.favor', ['product' => $product->id]) }}')
                .then(function () { // 请求成功会执行下面这个回调
                    swal('操作成功', '', 'success')
                    .then(function () { //这里加了一个 then() 方法，用户刷新
                        location.reload();
                    });
                }, function (error) { //请求失败会执行下面这个回调
                    // 如果返回码是 401 代表没有登录
